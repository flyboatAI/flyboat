...
from element_configuration.input_output_element.sync_input_configuration import (
    delete_sync_element_column_configuration_sql,
    delete_sync_element_configuration_sql,
    get_config_id,
    sync_element_column_configuration_dict_sql,
    sync_element_column_configuration_sql,
...
def get(prev_node_output_port, prev_node_type, prev_element_id, version_id, element_id):
    ...
    """
    sync_output_sql = (
        f"select t2.json_key as key, t2.nick_name "
        ...
        f"and t2.element_type='{ElementConfigType.Output.value}'"
    )
    sync_output_params_dict = db_helper1.fetchone(sync_output_sql, [])
    key = ""
    nick_name = ""
    if sync_output_params_dict:
        key = sync_output_params_dict["key"]
        nick_name = sync_output_params_dict["nick_name"]
    ...
    return execute_success(data={"key": key, "nick_name": nick_name, "fields": fields})


def configuration(version_id, element_id, user_id, key, nick_name, fields):
    """
    用于保存算子配置数据
    :param version_id: 版本标识
    :param element_id: 算子标识
    ...
    :return: 配置成功/失败
    """
    configuration_sql = sync_output_configuration_sql(version_id, element_id, user_id)
    ...
    config_configuration_sql = sync_element_config_configuration_sql(
        ...
        element_id,
        user_id,
        ElementConfigType.Output.value,
        key,
        nick_name=nick_name,
    )

    delete_sync_output_column_sql = delete_sync_element_column_configuration_sql(config_id)
    if fields and isinstance(fields, list):
        ...
        column_configuration_sql = sync_element_column_configuration_dict_sql(user_id, config_id, fields)
    else:
    ...
    return db_helper1.execute_arr(
        ...
        {0: [json.dumps([fields]) if fields else json.dumps([]), json.dumps([])]},
    )


def sync_output_configuration_sql(version_id, element_id, user_id):
    """
    生成插入或更新算子 SQL 语句
    :param version_id: 版本标识
    :param element_id: 算子标识
    ...
    # 插入或更新
    sql = (
        f"merge into ml_sync_output_element t1 "
        f"using (select "
        f"'{element_id}' id, "
...
def generate_init_sql(version_id, element_id, user_id, node_type):
    ...
    init_sql = (
        f"insert into ml_sync_output_element"
    ...
    init_config_sql = (
        ...
        f"'{ElementConfigType.Output.value}')"
    )
...
def generate_delete_sql(version_id, element_id, node_type):
    ...
    # 删除算子数据
    # noinspection SqlResolve
    delete_sql = f"delete from ml_sync_output_element " f"where version_id='{version_id}' " f"and id='{element_id}'"
