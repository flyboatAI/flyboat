...
def get(prev_node_output_port, prev_node_type, prev_element_id, version_id, element_id):
    ...
    box_plot_analyze_sql = (
        f"select dimension_column, value_column "
        f"from ml_box_plot_analyze_element "
        f"where version_id='{version_id}' "
        f"and id='{element_id}' "
        f"and is_enabled={Enabled.Yes.value}"
    )
    box_plot_analyze_dict = db_helper1.fetchone(box_plot_analyze_sql, [])
    if not box_plot_analyze_dict or (
        box_plot_analyze_dict["dimension_column"] is None and box_plot_analyze_dict["value_column"] is None
    ):
        return execute_success(data={"fields": fields, "dimension_column": None, "value_column": None})
    return execute_success(data={**{"fields": fields}, **box_plot_analyze_dict})
...
def configuration(
    ...
    :param prev_node_type: 所连接算子的类型
    :param prev_element_id: 所连接算子的标识
    :param dimension_column: dimension_column
    :param value_column: value_column
    :return: 配置成功/失败
    """
    query_result = get_fields_and_role(prev_node_output_port, prev_node_type, prev_element_id, version_id, db_helper1)
    if query_result.code != ResultCode.Success.value:
    ...
    configuration_sql = generate_configuration_sql(version_id, element_id, user_id, dimension_column, value_column)
...
def generate_configuration_sql(version_id, element_id, user_id, dimension_column, value_column):
    """
    生成插入或更新算子 SQL 语句
    :param version_id: 版本标识
    :param element_id: 算子标识
    ...
    """
    now = current_time()
    x_sql = f"'{dimension_column}' dimension_column" if dimension_column is not None else "null dimension_column"
    y_sql = f"'{value_column}' value_column" if value_column is not None else "null value_column"
    # 插入或更新
    sql = (
        f"merge into ml_box_plot_analyze_element t1 "
        f"using (select "
        f"'{element_id}' id, "
        ...
        f"version_id, dimension_column, value_column, "
        f"is_enabled) values"
        f"(t2.id, "
        f"t2.user_id, "
        f"to_date('{now}', 'yyyy-mm-dd hh24:mi:ss'), "
        f"t2.version_id, "
        f"t2.dimension_column, "
        f"t2.value_column, "
        f"{Enabled.Yes.value}) "
        f"when matched then "
        f"update set t1.dimension_column=t2.dimension_column, "
...
def generate_init_sql(version_id, element_id, user_id, node_type):
    ...
    init_sql = (
        f"insert into ml_box_plot_analyze_element"
        f"(id, create_user, create_time, "
        f"version_id, dimension_column, value_column, is_enabled)"
        f"values"
        f"('{element_id}', "
        f"'{user_id}', "
        f"to_date('{now}', 'yyyy-mm-dd hh24:mi:ss'), "
...
def generate_copy_sql(new_version_id, old_version_id):
    ...
    copy_sql = (
        ...
        f"'{new_version_id}', dimension_column, value_column, is_enabled "
        f"from ml_box_plot_analyze_element "
