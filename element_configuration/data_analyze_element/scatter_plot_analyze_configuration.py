...
def get(prev_node_output_port, prev_node_type, prev_element_id, version_id, element_id):
    ...
    scatter_plot_analyze_sql = (
        f"select x, y "
        f"from ml_scatter_plot_analyze_element "
        f"where version_id='{version_id}' "
        f"and id='{element_id}' "
        f"and is_enabled={Enabled.Yes.value}"
    )
    scatter_plot_analyze_dict = db_helper1.fetchone(scatter_plot_analyze_sql, [])
    if not scatter_plot_analyze_dict or (
        scatter_plot_analyze_dict["x"] is None and scatter_plot_analyze_dict["y"] is None
    ):
        return execute_success(data={"fields": fields, "x": None, "y": None})
    return execute_success(data={**{"fields": fields}, **scatter_plot_analyze_dict})
...
def generate_configuration_sql(version_id, element_id, user_id, x, y):
    ...
    # 插入或更新
    sql = (
        f"merge into ml_scatter_plot_analyze_element t1 "
        f"using (select "
        f"'{element_id}' id, "
...
def generate_init_sql(version_id, element_id, user_id, node_type):
    ...
    init_sql = (
        f"insert into ml_scatter_plot_analyze_element"
