...
def get(prev_node_output_port, prev_node_type, prev_element_id, version_id, element_id):
    ...
    pie_plot_analyze_dict = db_helper1.fetchone(pie_plot_analyze_sql, [])
    if not pie_plot_analyze_dict or (pie_plot_analyze_dict["dimension_mapping"] is None):
        return execute_success(data={"fields": fields, "dimension_mapping": None})
    return execute_success(data={**{"fields": fields}, **pie_plot_analyze_dict})
...
def generate_configuration_sql(version_id, element_id, user_id):
    ...
    # 插入或更新
    sql = (
        f"merge into ml_pie_plot_analyze_element t1 "
        f"using (select "
        f"'{element_id}' id, "
...
def generate_init_sql(version_id, element_id, user_id, node_type):
    ...
    init_sql = (
        f"insert into ml_pie_plot_analyze_element"
