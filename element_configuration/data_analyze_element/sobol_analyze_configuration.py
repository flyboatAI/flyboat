...
def get(prev_node_output_port, prev_node_type, prev_element_id, version_id, element_id):
    ...
    if not algorithm_dict or algorithm_dict["feature_value"] is None:
        for f in filter_fields:
            f["value"] = {
                "max": None,
                "min": None,
            }
        return execute_success(data={"feature_value": filter_fields})
    feature_value = algorithm_dict["feature_value"]
    return execute_success(data={"feature_value": json.loads(feature_value)})


def configuration(version_id, element_id, user_id, feature_value):
    """
    用于保存算子配置数据
    :param version_id: 版本标识
    :param element_id: 算子标识
    :param user_id: 用户表示
    :param feature_value: 特征值范围
    :return: 配置成功/失败
    """
    ...
    #     name = v.get('name')
    #     fields.append(generate_fields(str(name), data_type=UserDataType.Number.value))
    fields = [
...
def generate_configuration_sql(version_id, element_id, user_id):
    ...
    # 插入或更新
    configuration_sql = (
        f"merge into ml_sobol_analyze_element t1 "
        f"using (select "
        f"'{element_id}' id, "
...
def generate_init_sql(version_id, element_id, user_id, node_type):
    ...
    init_sql = (
        f"insert into ml_sobol_analyze_element"
...
def generate_copy_sql(new_version_id, old_version_id):
    ...
    copy_sql = (
        ...
        f"'{new_version_id}', feature_value, "
        f"is_enabled "
        f"from ml_sobol_analyze_element "
