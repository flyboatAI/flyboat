...
def get(prev_node_output_port, prev_node_type, prev_element_id, version_id, element_id):
    ...
    spider_plot_analyze_sql = (
        f"select dimension_mapping "
        f"from ml_spider_plot_analyze_element "
        f"where version_id='{version_id}' "
        f"and id='{element_id}' "
        f"and is_enabled={Enabled.Yes.value}"
    )
    spider_plot_analyze_dict = db_helper1.fetchone(spider_plot_analyze_sql, [])
    if not spider_plot_analyze_dict or (spider_plot_analyze_dict["dimension_mapping"] is None):
        return execute_success(data={"fields": fields, "dimension_mapping": None})
    return execute_success(data={**{"fields": fields}, **spider_plot_analyze_dict})
...
def configuration(
    ...
    :param prev_node_type: 所连接算子的类型
    :param prev_element_id: 所连接算子的标识
    :param dimension_mapping: 维度与映射列表
    :return: 配置成功/失败
    """
    query_result = get_fields_and_role(prev_node_output_port, prev_node_type, prev_element_id, version_id, db_helper1)
    if query_result.code != ResultCode.Success.value:
...
def generate_configuration_sql(version_id, element_id, user_id):
    ...
    # 插入或更新
    sql = (
        f"merge into ml_spider_plot_analyze_element t1 "
        f"using (select "
        f"'{element_id}' id, "
        ...
        f"version_id, dimension_mapping, "
        f"is_enabled) values"
        f"(t2.id, "
        f"t2.user_id, "
        ...
        f"{Enabled.Yes.value}) "
        f"when matched then "
        f"update set t1.dimension_mapping=:2, "
...
def generate_init_sql(version_id, element_id, user_id, node_type):
    ...
    init_sql = (
        f"insert into ml_spider_plot_analyze_element"
        f"(id, create_user, create_time, "
        f"version_id, dimension_mapping, is_enabled)"
        f"values"
        f"('{element_id}', "
        f"'{user_id}', "
        f"to_date('{now}', 'yyyy-mm-dd hh24:mi:ss'), "
...
def generate_copy_sql(new_version_id, old_version_id):
    ...
    copy_sql = (
        ...
        f"'{new_version_id}', dimension_mapping, is_enabled "
        f"from ml_spider_plot_analyze_element "
