...
def get(prev_node_output_port, prev_node_type, prev_element_id, version_id, element_id):
    ...
    if not algorithm_dict:
        return execute_success(data={"l": 1, "weight": [], "role": role})
    weight = json.loads(algorithm_dict["weight"])
    algorithm_dict["weight"] = weight
    algorithm_dict["role"] = role
...
def configuration(
    ...
    configuration_sql = generate_configuration_sql(version_id, element_id, user_id, L)
...
def generate_configuration_sql(version_id, element_id, user_id, L):
    """
    生成插入或更新算子 SQL 语句
    :param version_id: 版本标识
    :param element_id: 算子标识
    ...
    # 插入或更新
    sql = (
        f"merge into ml_analogy_estimation_element t1 "
        f"using (select "
        f"'{element_id}' id, "
...
def generate_init_sql(version_id, element_id, user_id, node_type):
    ...
    init_sql = (
        f"insert into ml_analogy_estimation_element"
        f"(id, create_user, create_time, "
        f"version_id, L, weight, "
        f"is_enabled)"
        f"values"
        f"('{element_id}', "
...
def generate_copy_sql(new_version_id, old_version_id):
    ...
    copy_sql = (
        ...
        f"'{new_version_id}', L, weight, is_enabled "
        f"from ml_analogy_estimation_element "
