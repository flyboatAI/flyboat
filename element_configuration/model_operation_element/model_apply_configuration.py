...
def configuration(version_id, element_id, user_id, fields):
    """
    用于保存算子配置数据
    :param version_id: 版本标识
    :param element_id: 算子标识
    ...
    return db_helper1.execute_arr(
        ...
        {0: [fields_arr_json, role_arr_json], 1: [fields_json, fields_json]},
    )
...
def generate_configuration_sql(version_id, element_id, user_id):
    ...
    # 插入或更新
    configuration_sql = (
        f"merge into ml_model_apply_element t1 "
        f"using (select "
        f"'{element_id}' id, "
...
def generate_init_sql(version_id, element_id, user_id, node_type):
    ...
    init_sql = (
        f"insert into ml_model_apply_element"
...
def generate_copy_sql(new_version_id, old_version_id):
    ...
    copy_sql = (
        ...
        f"'{new_version_id}', fields, is_enabled "
        f"from ml_model_apply_element "
