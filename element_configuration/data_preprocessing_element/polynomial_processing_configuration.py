...
from helper.polynomial_processing_helper import polynomial_format
...
def get(version_id, element_id):
    ...
    if (
        not polynomial_processing_dict
        or (polynomial_processing_dict["degree"] is None)
        or (polynomial_processing_dict["interaction_only"] is None)
    ):
        return execute_success(data={"degree": 2, "interaction_only": False})
    degree = polynomial_processing_dict["degree"]
    interaction_only = polynomial_processing_dict["interaction_only"]
    return execute_success(data={"degree": degree, "interaction_only": interaction_only})
...
def configuration(
    ...
    :param degree: 多项式乘阶
    :param interaction_only: 是否包含特征和自己相乘的项
    :return: 配置成功/失败
    """
    ...
    new_fields = polynomial_format(fields, degree, interaction_only)
    ...
    if degree is None or interaction_only is None:
        degree = 2
        interaction_only = 0
    configuration_sql = generate_configuration_sql(version_id, element_id, user_id, degree, interaction_only)
...
def generate_configuration_sql(version_id, element_id, user_id, degree, interaction_only):
    """
    生成插入或更新算子 SQL 语句
    :param interaction_only: interaction_only
    :param degree: degree
    :param version_id: 版本标识
    :param element_id: 算子标识
    :param user_id: 用户标识
    ...
    """
    now = current_time()
    int_interaction_only = 1 if interaction_only else 0
    # 插入或更新
    sql = (
        f"merge into ml_polynomial_processing_element t1 "
        f"using (select "
        f"'{element_id}' id, "
        f"'{version_id}' version_id, "
        f"'{user_id}' user_id, "
        f"{degree} degree, "
        f"{int_interaction_only} interaction_only "
        f"from dual) t2 on (t1.id = t2.id "
        f"and t1.version_id=t2.version_id) "
        f"when not matched then "
        f"insert (id, create_user, create_time, "
        f"version_id, degree, interaction_only, "
        f"is_enabled) values"
        f"(t2.id, "
        f"t2.user_id, to_date('{now}', 'yyyy-mm-dd hh24:mi:ss'), "
        f"t2.version_id, "
        f"t2.degree, t2.interaction_only, "
        f"{Enabled.Yes.value}) "
        f"when matched then "
        f"update set t1.degree=t2.degree, "
        f"t1.interaction_only=t2.interaction_only, "
...
def generate_init_sql(version_id, element_id, user_id, node_type):
    ...
    init_sql = (
        f"insert into ml_polynomial_processing_element"
...
def generate_copy_sql(new_version_id, old_version_id):
    ...
    copy_sql = (
        ...
        f"'{new_version_id}', degree, interaction_only, is_enabled "
        f"from ml_polynomial_processing_element "
