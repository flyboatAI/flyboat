...
def get(prev_node_output_port, prev_node_type, prev_element_id, version_id, element_id):
    ...
    :return: {
                "fields": [],
                "filter_type": "all",
                "filter_compare_fields": [{"name": "", "value": 0, "compare_type": "less_than"}]
              }
    """
    ...
    data_filter_sql = (
        f"select filter_type, filter_compare_fields "
    ...
    if not data_filter_dict or (
        data_filter_dict["filter_type"] is None and data_filter_dict["filter_compare_fields"] is None
    ):
        return execute_success(
            data={
                "fields": fields,
                "filter_type": FilterType.All.value,
                "filter_compare_fields": [],
            }
        )
    filter_compare_fields_json = data_filter_dict["filter_compare_fields"]
    if filter_compare_fields_json:
        filter_compare_fields = json.loads(filter_compare_fields_json)
        data_filter_dict["filter_compare_fields"] = filter_compare_fields
    data_filter_dict["fields"] = fields
    return execute_success(data=data_filter_dict)
...
def configuration(
    ...
    :param filter_compare_fields: 过滤比较字段、类型、值数组
    :return: 配置成功/失败
    """
    query_result = get_fields_and_role(prev_node_output_port, prev_node_type, prev_element_id, version_id, db_helper1)
    if query_result.code != ResultCode.Success.value:
    ...
    if filter_compare_fields is None:
        filter_compare_fields = []
    filter_compare_fields_json = json.dumps(filter_compare_fields)
    # 点击确定按钮，保存输出端口信息和配置信息至记录表和配置表中
    output_configuration_store_sql = output_port_record_sql(version_id, element_id, user_id)
    configuration_sql = generate_configuration_sql(version_id, element_id, user_id, filter_type)
    return db_helper1.execute_arr(
        [output_configuration_store_sql, configuration_sql],
        {
            0: [fields_arr_json, role_arr_json],
            1: [filter_compare_fields_json, filter_compare_fields_json],
        },
    )


def generate_configuration_sql(version_id, element_id, user_id, filter_type):
    """
    生成插入或更新算子 SQL 语句
    :param version_id: 版本标识
    :param element_id: 算子标识
    ...
    # 插入或更新
    sql = (
        f"merge into ml_data_filter_element t1 "
        f"using (select "
        f"'{element_id}' id, "
        ...
        f"version_id, filter_type, filter_compare_fields, "
        f"is_enabled) values"
        f"(t2.id, "
        ...
        f"{Enabled.Yes.value}) "
        f"when matched then "
        f"update set t1.filter_type=t2.filter_type, "
        f"t1.filter_compare_fields=:2, "
...
def generate_init_sql(version_id, element_id, user_id, node_type):
    ...
    init_sql = (
        f"insert into ml_data_filter_element"
...
def generate_copy_sql(new_version_id, old_version_id):
    ...
    copy_sql = (
        ...
        f"'{new_version_id}', filter_type, filter_compare_fields, is_enabled "
        f"from ml_data_filter_element "
