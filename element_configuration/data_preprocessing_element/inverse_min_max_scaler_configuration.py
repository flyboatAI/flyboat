...
def get(prev_node_output_port, prev_node_type, prev_element_id, version_id, element_id):
    ...
    inverse_min_max_fields_dict = db_helper1.fetchone(inverse_min_max_sql, [])
    if not inverse_min_max_fields_dict or (inverse_min_max_fields_dict["inverse_min_max_fields"] is None):
        return execute_success(data={"fields": fields, "inverse_min_max_fields": []})
    inverse_min_max_fields_json = inverse_min_max_fields_dict["inverse_min_max_fields"]
    if inverse_min_max_fields_json:
        inverse_min_max_fields = json.loads(inverse_min_max_fields_json)
        return execute_success(data={"fields": fields, "inverse_min_max_fields": inverse_min_max_fields})
    return execute_success(data={"fields": fields, "inverse_min_max_fields": []})
...
def configuration(
    ...
    :param prev_node_type: 所连接算子的类型
    :param prev_element_id: 所连接算子的标识
    :param inverse_min_max_fields: 逆归一化字段
    :return: 配置成功/失败
    """
    query_result = get_fields_and_role(prev_node_output_port, prev_node_type, prev_element_id, version_id, db_helper1)
    if query_result.code != ResultCode.Success.value:
    ...
    if inverse_min_max_fields is None:
        inverse_min_max_fields = []
    inverse_min_max_fields_json = json.dumps(inverse_min_max_fields)
    ...
    return db_helper1.execute_arr(
        [output_configuration_store_sql, configuration_sql],
        {
            ...
            1: [inverse_min_max_fields_json, inverse_min_max_fields_json],
        },
    )
...
def generate_configuration_sql(version_id, element_id, user_id):
    ...
    # 插入或更新
    sql = (
        f"merge into ml_inverse_min_max_scaler_element t1 "
        f"using (select "
        f"'{element_id}' id, "
        ...
        f"version_id, inverse_min_max_fields, "
        f"is_enabled) values"
        f"(t2.id, "
        ...
        f"{Enabled.Yes.value}) "
        f"when matched then "
        f"update set t1.inverse_min_max_fields=:2, "
...
def generate_init_sql(version_id, element_id, user_id, node_type):
    ...
    init_sql = (
        f"insert into ml_inverse_min_max_scaler_element"
...
def generate_copy_sql(new_version_id, old_version_id):
    ...
    copy_sql = (
        ...
        f"'{new_version_id}', inverse_min_max_fields, is_enabled "
        f"from ml_inverse_min_max_scaler_element "
