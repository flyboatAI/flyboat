...
def get(prev_node_output_port, prev_node_type, prev_element_id, version_id, element_id):
    ...
    min_max_fields_dict = db_helper1.fetchone(min_max_sql, [])
    if not min_max_fields_dict or (min_max_fields_dict["min_max_fields"] is None):
        return execute_success(data={"fields": fields, "min_max_fields": []})
    min_max_fields_json = min_max_fields_dict["min_max_fields"]
    if min_max_fields_json:
        min_max_fields = json.loads(min_max_fields_json)
        return execute_success(data={"fields": fields, "min_max_fields": min_max_fields})
    return execute_success(data={"fields": fields, "min_max_fields": []})
...
def configuration(
    ...
    if min_max_fields is None:
        min_max_fields = []
    min_max_fields_json = json.dumps(min_max_fields)
    ...
    return db_helper1.execute_arr(
        [output_configuration_store_sql, configuration_sql],
        {
            ...
            1: [min_max_fields_json, min_max_fields_json],
        },
    )
...
def generate_configuration_sql(version_id, element_id, user_id):
    ...
    # 插入或更新
    sql = (
        f"merge into ml_min_max_scaler_element t1 "
        f"using (select "
        f"'{element_id}' id, "
        ...
        f"{Enabled.Yes.value}) "
        f"when matched then "
        f"update set t1.min_max_fields=:2, "
...
def generate_init_sql(version_id, element_id, user_id, node_type):
    ...
    init_sql = (
        f"insert into ml_min_max_scaler_element"
...
def generate_copy_sql(new_version_id, old_version_id):
    ...
    copy_sql = (
        ...
        f"'{new_version_id}', min_max_fields, is_enabled "
        f"from ml_min_max_scaler_element "
