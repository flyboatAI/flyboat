...
def get(version_id, element_id):
    ...
    pca_decomposition_dict = db_helper1.fetchone(pca_decomposition_sql, [])
    if not pca_decomposition_dict or (pca_decomposition_dict["n_components"] is None):
        return execute_success(data={"n_components": 2})
    n_components = pca_decomposition_dict["n_components"]
    return execute_success(data={"n_components": n_components})


def configuration(version_id, element_id, user_id, n_components):
    """
    用于保存算子配置数据
    :param n_components: 降维的数量
    :param version_id: 版本标识
    :param element_id: 算子标识
    :param user_id: 用户表示
    :return: 配置成功/失败
    """
    fields = [
        generate_fields(name, nick_name=name, data_type=UserDataType.Number.value)
        for name in [f"x_{i}" for i in range(1, n_components + 1)]
    ]
    ...
    # 点击确定按钮，保存输出端口信息和配置信息至记录表和配置表中
    configuration_sql = generate_configuration_sql(version_id, element_id, user_id, n_components)
...
def generate_configuration_sql(version_id, element_id, user_id, n_components):
    """
    生成插入或更新算子 SQL 语句
    :param n_components: 降维的数量
    :param version_id: 版本标识
    ...
    # 插入或更新
    sql = (
        f"merge into ml_pca_decomposition_element t1 "
        f"using (select "
        f"'{element_id}' id, "
        ...
        f"{Enabled.Yes.value}) "
        f"when matched then "
        f"update set t1.n_components=t2.n_components, "
...
def generate_init_sql(version_id, element_id, user_id, node_type):
    ...
    init_sql = (
        f"insert into ml_pca_decomposition_element"
...
def generate_copy_sql(new_version_id, old_version_id):
    ...
    copy_sql = (
        ...
        f"'{new_version_id}', n_components, is_enabled "
        f"from ml_pca_decomposition_element "
