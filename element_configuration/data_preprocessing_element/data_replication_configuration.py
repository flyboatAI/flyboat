...
def get(prev_node_output_port, prev_node_type, prev_element_id, version_id, element_id):
    ...
    if not data_replication_dict or data_replication_dict["output_port_num"] is None:
        return execute_success(data={"output_port_num": 1})
    return execute_success(data=data_replication_dict)
...
def configuration(
    ...
    if isinstance(output_port_num, str):
        output_port_num = int(output_port_num)
    fields_arr = [fields] * output_port_num
    role_arr = [role] * output_port_num
    ...
    configuration_sql = generate_configuration_sql(version_id, element_id, user_id, output_port_num)
...
def generate_configuration_sql(version_id, element_id, user_id, output_port_num):
    """
    生成插入或更新算子 SQL 语句
    :param version_id: 版本标识
    :param element_id: 算子标识
    ...
    # 插入或更新
    configuration_sql = (
        f"merge into ml_data_replication_element t1 "
        f"using (select "
        f"'{element_id}' id, "
        f"'{version_id}' version_id, "
        f"{output_port_num} output_port_num, "
        f"'{user_id}' user_id from dual) t2 on (t1.id = t2.id "
        f"and t1.version_id=t2.version_id) "
        f"when not matched then "
        f"insert (id, create_user, create_time, "
        f"version_id, output_port_num, "
        f"is_enabled) values"
        f"(t2.id, "
        f"t2.user_id, to_date('{now}', 'yyyy-mm-dd hh24:mi:ss'), "
        f"t2.version_id, "
        f"t2.output_port_num, "
        f"{Enabled.Yes.value}) "
        f"when matched then "
        f"update set t1.output_port_num=t2.output_port_num, "
...
def generate_init_sql(version_id, element_id, user_id, node_type):
    ...
    init_sql = (
        f"insert into ml_data_replication_element"
...
def generate_copy_sql(new_version_id, old_version_id):
    ...
    copy_sql = (
        ...
        f"'{new_version_id}', output_port_num, is_enabled "
        f"from ml_data_replication_element "
