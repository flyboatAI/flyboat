...
def get(prev_node_output_port, prev_node_type, prev_element_id, version_id, element_id):
    ...
    :return: {
                "fields": [],
                "process_fields": [{"name": "", "process_type": "boxcox"}]
              }
    """
    ...
    if not data_process_dict or data_process_dict["process_fields"] is None:
        return execute_success(data={"fields": fields, "process_fields": []})
    process_fields_json = data_process_dict["process_fields"]
    if process_fields_json:
        process_fields = json.loads(process_fields_json)
        data_process_dict["process_fields"] = process_fields
    data_process_dict["fields"] = fields
    return execute_success(data=data_process_dict)
...
def configuration(
    ...
    if process_fields is None:
        process_fields = []
    empty_process_type_len = len([field for field in process_fields if not field["process_type"]])
    if empty_process_type_len:
        raise ElementConfigurationConfigError(sys._getframe().f_code.co_name, "列数据处理类型为空") from None
    process_fields_json = json.dumps(process_fields)
    ...
    return db_helper1.execute_arr(
        [output_configuration_store_sql, configuration_sql],
        {
            ...
            1: [process_fields_json, process_fields_json],
        },
    )
...
def generate_configuration_sql(version_id, element_id, user_id):
    ...
    # 插入或更新
    configuration_sql = (
        f"merge into ml_column_data_process_element t1 "
        f"using (select "
        f"'{element_id}' id, "
        ...
        f"{Enabled.Yes.value}) "
        f"when matched then "
        f"update set t1.process_fields=:2, "
...
def generate_init_sql(version_id, element_id, user_id, node_type):
    ...
    init_sql = (
        f"insert into ml_column_data_process_element"
...
def generate_copy_sql(new_version_id, old_version_id):
    ...
    copy_sql = (
        ...
        f"'{new_version_id}', process_fields, is_enabled "
        f"from ml_column_data_process_element "
