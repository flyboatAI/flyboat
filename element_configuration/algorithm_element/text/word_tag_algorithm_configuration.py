...
def get(prev_node_output_port, prev_node_type, prev_element_id, version_id, element_id):
    ...
    if not word_tag_dict or (word_tag_dict["column_name"] is None and not word_tag_dict["top_k"]):
        return execute_success(data={"fields": fields, "top_k": 5, "column_name": ""})
    column_name = word_tag_dict["column_name"]
    top_k = word_tag_dict["top_k"]
    return execute_success(data={"fields": fields, "top_k": top_k, "column_name": column_name})
...
def configuration(
    ...
    # 点击确定按钮，保存输出端口信息和配置信息至记录表和配置表中
    configuration_sql = generate_configuration_sql(version_id, element_id, user_id, top_k, column_name)
...
def generate_configuration_sql(version_id, element_id, user_id, top_k, column_name):
    """
    生成插入或更新算子 SQL 语句
    :param column_name: 分词列
    :param top_k: 提取数量
    ...
    # 插入或更新
    sql = (
        f"merge into ml_text_tag_element t1 "
        f"using (select "
        f"'{element_id}' id, "
        ...
        f"version_id, top_k, column_name, "
        f"is_enabled) values"
        f"(t2.id, "
...
def generate_init_sql(version_id, element_id, user_id, node_type):
    ...
    init_sql = (
        f"insert into ml_text_tag_element"
...
def generate_copy_sql(new_version_id, old_version_id):
    ...
    copy_sql = (
        ...
        f"'{new_version_id}', top_k, column_name, is_enabled "
        f"from ml_text_tag_element "
