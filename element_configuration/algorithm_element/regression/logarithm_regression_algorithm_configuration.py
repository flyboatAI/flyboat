...
def get(version_id, element_id):
    ...
    if not algorithm_dict:
        return execute_success(data={"a": 1, "b": 2})

    return execute_success(data=algorithm_dict)


def configuration(version_id, element_id, user_id, a, b):
    """
    用于保存算子配置数据
    :param version_id: 版本标识
    :param element_id: 算子标识
    :param user_id: 用户表示
    :param a: a + b * np.log10(x) 中的a
    :param b: a + b * np.log10(x) 中的b
    :return: 配置成功/失败
    """
    # 点击确定按钮，保存输出端口信息和配置信息至记录表和配置表中
    configuration_sql = generate_configuration_sql(version_id, element_id, user_id, a, b)
    return db_helper1.execute_arr([configuration_sql])


def generate_configuration_sql(version_id, element_id, user_id, a, b):
    """
    生成插入或更新算子 SQL 语句
    :param version_id: 版本标识
    :param element_id: 算子标识
    ...
    # 插入或更新
    configuration_sql = (
        f"merge into ml_logarithm_regression_element t1 "
        f"using (select "
        f"'{element_id}' id, "
...
def generate_init_sql(version_id, element_id, user_id, node_type):
    ...
    init_sql = (
        f"insert into ml_logarithm_regression_element"
...
def generate_copy_sql(new_version_id, old_version_id):
    ...
    copy_sql = (
        ...
        f"'{new_version_id}', a, b, is_enabled "
        f"from ml_logarithm_regression_element "
