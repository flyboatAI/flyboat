...
def configuration(version_id, element_id, user_id, epoch, lr, hidden_size, num_layers, dropout):
    """
    用于保存算子配置数据
    :param version_id: 版本标识
    :param element_id: 算子标识
...
def generate_configuration_sql(version_id, element_id, user_id, epoch, lr, hidden_size, num_layers, dropout):
    """
    生成插入或更新算子 SQL 语句
    :param version_id: 版本标识
    :param element_id: 算子标识
    ...
    # 插入或更新
    configuration_sql = (
        f"merge into ml_lstm_regression_element t1 "
        f"using (select "
        f"'{element_id}' id, "
        ...
        f"epoch, lr,hidden_size,num_layers,dropout, is_enabled) "
        f"values"
        f"(t2.id, "
        ...
        f"t1.epoch=t2.epoch, "
        f"t1.lr=t2.lr, "
        f"t1.hidden_size=t2.hidden_size, "
        f"t1.num_layers=t2.num_layers, "
        f"t1.dropout=t2.dropout, "
...
def generate_init_sql(version_id, element_id, user_id, node_type):
    ...
    init_sql = (
        f"insert into ml_lstm_regression_element"
        f"(id, create_user, create_time, "
        f"version_id, epoch, lr, hidden_size, num_layers, dropout, "
        f"is_enabled)"
        f"values"
        f"('{element_id}', "
        f"'{user_id}', "
...
def generate_copy_sql(new_version_id, old_version_id):
    ...
    copy_sql = (
        ...
        f"'{new_version_id}', epoch, lr, hidden_size, num_layers, dropout, is_enabled "
        f"from ml_lstm_regression_element "
