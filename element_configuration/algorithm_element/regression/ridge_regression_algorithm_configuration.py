...
def get(version_id, element_id):
    ...
    if not algorithm_dict:
        return execute_success(data={"alpha": 100})

    return execute_success(data=algorithm_dict)


def configuration(version_id, element_id, user_id, alpha):
    """
    用于保存算子配置数据
    :param version_id: 版本标识
    :param element_id: 算子标识
    ...
    configuration_sql = generate_configuration_sql(version_id, element_id, user_id, alpha)
    return db_helper1.execute_arr([configuration_sql])


def generate_configuration_sql(version_id, element_id, user_id, alpha):
    """
    生成插入或更新算子 SQL 语句
    :param version_id: 版本标识
    :param element_id: 算子标识
    ...
    # 插入或更新
    configuration_sql = (
        f"merge into ml_ridge_regression_element t1 "
        f"using (select "
        f"'{element_id}' id, "
...
def generate_init_sql(version_id, element_id, user_id, node_type):
    ...
    init_sql = (
        f"insert into ml_ridge_regression_element"
...
def generate_copy_sql(new_version_id, old_version_id):
    ...
    copy_sql = (
        ...
        f"'{new_version_id}', alpha, is_enabled "
        f"from ml_ridge_regression_element "
