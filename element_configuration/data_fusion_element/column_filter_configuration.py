...
from helper.column_filter_helper import role_filter
...
def get(prev_node_output_port, prev_node_type, prev_element_id, version_id, element_id):
    ...
    :return: {
                ...
                "filter_fields": [{"name": "", "nick_name": "", "data_type": "NUMBER"}]
              }
    """
    ...
    if not column_filter_dict or (column_filter_dict["filter_fields"] is None):
        return execute_success(data={"fields": fields, "filter_fields": []})
    filter_fields_json = column_filter_dict["filter_fields"]
    if filter_fields_json:
        filter_fields = json.loads(filter_fields_json)
        return execute_success(data={"fields": fields, "filter_fields": filter_fields})
    return execute_success(data={"fields": fields, "filter_fields": []})
...
def configuration(
    ...
    filter_role = role_filter(role, filter_fields) if role is not None else None
    ...
    return db_helper1.execute_arr(
        [output_configuration_store_sql, configuration_sql],
        {
            ...
            1: [filter_fields_json, filter_fields_json],
        },
    )
...
def generate_configuration_sql(version_id, element_id, user_id):
    ...
    # 插入或更新
    sql = (
        f"merge into ml_column_filter_element t1 "
        f"using (select "
        f"'{element_id}' id, "
        ...
        f"{Enabled.Yes.value}) "
        f"when matched then "
        f"update set t1.filter_fields=:2, "
...
def generate_init_sql(version_id, element_id, user_id, node_type):
    ...
    init_sql = (
        f"insert into ml_column_filter_element"
...
def generate_copy_sql(new_version_id, old_version_id):
    ...
    copy_sql = (
        ...
        f"'{new_version_id}', filter_fields, is_enabled "
        f"from ml_column_filter_element "
