...
class FactorAnalyze(AbstractElement):
    def element_process(
        self,
        process_id,
        dependency_id_arr,
        ...
        **kwargs,
        ...
        # 成分得分系数表
        component_table = md.component_table()
        # 线性组合系数矩阵
        liner_table = md.liner_coefficient_table()

        # 碎石图
        eigenvalue = md.get_eigenvalue()
        store_sql = process_data_store(process_id, self.v_id, self.e_id, self.u_id, StoreType.Factor.value)
        eigenvalue_fields = [
        ...
        component_table_relative_path = self.create_csv_file(component_table[0], component_table[1])
        ...
        path_dict = {
            ...
            "component_table": component_table_relative_path,
        ...
        factor_fields = {
            ...
            "component_table": component_table[1],
            "eigenvalue": eigenvalue_fields,
        }
        factor_fields_json = json.dumps(factor_fields)
        store_result = self.insert_process_pipelining(store_sql, [json.dumps(path_dict), factor_fields_json])
...
class FactorAnalysis:
    ...
    def variance_table(self):
        ...
        variance_data["特征根"] = self.featValue
        variance_data["方差解释率"] = np.round((self.featValue / np.sum(self.featValue)) * 100, 3)
        variance_data["累积方差解释率"] = np.cumsum(np.round((self.featValue / np.sum(self.featValue)) * 100, 3))

        variance_data["旋转前主成分_特征根"][: self.n] = np.around(fa.get_factor_variance()[0], 3)
        variance_data["旋转前主成分_方差解释率"][: self.n] = np.around(fa.get_factor_variance()[1] * 100, 3)
        variance_data["旋转前主成分_累积方差解释率"][: self.n] = np.around(fa.get_factor_variance()[2] * 100, 3)

        fa = FactorAnalyzer(n_factors=self.n, rotation="varimax", method="principal")
        fa.fit(self.data)
        # 排序
        variance_data["旋转后主成分_特征根"][: self.n] = np.around(fa.get_factor_variance()[0], 3)
        variance_data["旋转后主成分_方差解释率"][: self.n] = np.around(fa.get_factor_variance()[1] * 100, 3)
        variance_data["旋转后主成分_累积方差解释率"][: self.n] = np.around(fa.get_factor_variance()[2] * 100, 3)
    ...
    def component_table(self):
        ...
        # fa = FactorAnalyzer(n_factors=self.n, rotation=None, method='principal')
        fa.fit(self.data)
        f_corr = self.data.corr()  # 皮尔逊相关系数
        X1 = np.mat(f_corr)
        X1 = np.linalg.inv(X1)
        col_name_list = ["主成分" + str(x + 1) for x in range(self.n)]
        component_matrix = pd.DataFrame(np.around(np.dot(X1, fa.loadings_), 3), columns=col_name_list)
        component_matrix["特征"] = self.data.columns
        component_matrix = pd.concat([component_matrix["特征"], component_matrix.drop(columns="特征")], axis=1)
        component_matrix = matrix_data_format_conversion(component_matrix)
