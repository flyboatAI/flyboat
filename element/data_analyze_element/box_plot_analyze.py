...
class BoxPlotAnalyze(AbstractElement):
    def element_process(
        self,
        process_id,
        dependency_id_arr,
        ...
        **kwargs,
        ...
        box_sql = (
            f"select dimension_column, value_column, is_enabled "
        ...
        box_sql_dict = db_helper1.fetchone(box_sql, [])
        if "dimension_column" not in box_sql_dict or "value_column" not in box_sql_dict:
            raise ExecuteError(sys._getframe().f_code.co_name, f"{element_name}未配置完毕") from None
        dimension_column = box_sql_dict["dimension_column"]
        value_column = box_sql_dict["value_column"]
        if not dimension_column or not value_column:
            raise ExecuteError(sys._getframe().f_code.co_name, f"{element_name}维度列或值列未配置") from None

        data = port(0, data_arr)

        store_sql = process_data_store(process_id, self.v_id, self.e_id, self.u_id, StoreType.Box.value)
        relative_path = self.create_csv_file(data)
        fields_json = json.dumps({"dimension_column": dimension_column, "value_column": value_column})
